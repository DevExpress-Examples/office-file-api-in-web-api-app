...
namespace DocumentProcessingWebAPI.Controllers
{
    ...
    public class RichEditController : ControllerBase
    {
        ...
        public async Task<IActionResult> Encrypt(IFormFile file, [FromQuery] string password)
        {
            try
            {
                using (var stream = new MemoryStream())
                {
                    ...
                    {
                        server.LoadDocument(stream);
                        DevExpress.XtraRichEdit.API.Native.EncryptionSettings encryptionSettings = new DevExpress.XtraRichEdit.API.Native.EncryptionSettings();
        ...
        public async Task<IActionResult> MailMerge([FromQuery] MergeMode mergeMode, [FromQuery] RichEditFormat format)
        {
            try
            {
                using (var stream = new MemoryStream())
                {
                    ...
                    using (RichEditDocumentServer wordProcessor = new RichEditDocumentServer())
                    {
                        ...
                        Stream result = RichEditHelper.MailMerge(wordProcessor, format, myMergeOptions);
                        string contentType = RichEditHelper.GetContentType(format);
                        return File(result, contentType, $"result.{format}");
                    }
            ...
            try
            {
                using (var stream = new MemoryStream())
                {
                    ...
                    using (RichEditDocumentServer server = new RichEditDocumentServer())
                    {
                        ...
                        Stream result = RichEditHelper.ExportToAccessiblePdf(server, format);
